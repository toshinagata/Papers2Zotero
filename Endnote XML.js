{
	...
	"translatorType": 3,
	"browserSupport": "gcv",
	"lastUpdated": "2020-04-13 14:30:53"
}
...
function importNext(records, index, resolve, reject) {
	try {
		...
		for (var i = index, n = records.length; i < n; i++) {
			...
			if (record.children[0].nodeName == "create-collections") {
				var entries = record.children[0].children;
				var collectionCache = {};
				for (var k = 0; k < entries.length; k++) {
					var entry = entries[k];
					if (entry.nodeName == "collection") {
						var name = undefined;
						var parent = undefined;
						var items = [];
						for (var kk = 0; kk < entry.children.length; kk++) {
							var child = entry.children[kk];
							if (child.nodeName == "name") {
								name = child.textContent;
							} else if (child.nodeName == "parent") {
								parent = child.textContent;
							} else if (child.nodeName == "item") {
								items.push({type:"item", id:child.textContent});
							}
						}
						var col = new Zotero.Collection();
						col.name = name;
						collectionCache[col.name] = col;
						col.type = "collection";
						col.itemID = "collection" + k;
						col.children = items;
						col.parent = parent;
					}
				}
				for (var key in collectionCache) {
					var col = collectionCache[key];
					if (col.parent) {
						var parent_col = collectionCache[col.parent];
						if (parent_col) {
							parent_col.children.push(col);
						}
					}
				}
				for (var key in collectionCache) {
			...
			for (var j = 0; j < record.children.length; j++) {
				...
				} else if (field == "database" || field == "source-app" || field == "ref-type" 
					|| field == "foreign-keys" || field == "label" || field == "publisher"){
						//skipping these fields
				} else if (field == "rec-number") {
					newItem.itemID = node.textContent;
				} else {
